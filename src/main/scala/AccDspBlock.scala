...
abstract class AccumulatorBlock[T <: Data: Real: BinaryRepresentation, D, U, E, O, B <: Data](
  params:    AccParams[T],
  beatBytes: Int)
...
class AXI4AccumulatorBlock[T <: Data: Real: BinaryRepresentation](
  params:     AccParams[T],
  address:    AddressSet,
  _beatBytes: Int = 4
)(
  implicit p: Parameters)
    extends AccumulatorBlock[
      T,
...
class TLAccumulatorBlock[T <: Data: Real: BinaryRepresentation](
  val params: AccParams[T],
  address:    AddressSet,
  beatBytes:  Int = 4
)(
  implicit p: Parameters)
    extends AccumulatorBlock[T, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle](
      params,
      beatBytes
    )
    with TLDspBlock
...
//(new ChiselStage)
//  .execute(
//    Array("--target-dir", "verilog/AXI4AccumulatorBlock"),
//    Seq(ChiselGeneratorAnnotation(() => testModule.module))
//  )
// }
