...
abstract class MemForTestingAcc[T <: Data: Real: BinaryRepresentation, D, U, E, O, B <: Data](
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val realSin =
      (0 until memSize).map(i => Complex(((math.sin(2 * math.Pi * f1 * i)) * (math.pow(2, 13) - 1)), 0.0)).toVector
    // MSB byte, LSB byte
    // real part // imag part

    (0 until memSize).map(n => {
      rom(n).real := Real[T].fromDouble(realSin(n).real.toInt)
      rom(n).imag := Real[T].fromDouble(realSin(n).imag.toInt)
    })
    ...
    val fields = Seq(
      // settable registers
      RegField(1, startReading, RegFieldDesc(name = "startReading", desc = "enable reading from the memory")),
      RegField(1, runLast, RegFieldDesc(name = "runLast", desc = "set last signal"))
    )
...
class AXI4MemForTestingAcc[T <: Data: Real: BinaryRepresentation](
  proto:     DspComplex[T],
  address:   AddressSet,
  beatBytes: Int = 4,
  memSize:   Int = 1024
...
object MemForTestingAccAp extends App {
  ...
  val memTest = LazyModule(
    new AXI4MemForTestingAcc(
      DspComplex(FixedPoint(16.W, 0.BP)),
      AddressSet(0x000000, 0xff),
      beatBytes = 4,
      memSize = 1024
    ) with AXI4Block {
  ...
  (new ChiselStage)
    .execute(
      Array("--target-dir", "verilog/AXI4MemForTestingAcc"),
      Seq(ChiselGeneratorAnnotation(() => memTest.module))
    )
}
