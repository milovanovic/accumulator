...
class AccChainTester(
  ...
  // Connect AXI4StreamModel to DUT
   ...
   master.addTransactions(dataIn1.zipWithIndex.map { case (dataSample, idx) => AXI4StreamTransaction(data = dataSample,  last = if (idx == dataIn1.length - 1) true else false) })
